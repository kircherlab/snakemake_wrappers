...
@click.option('--variants',
              ...
              help='Variant file to predict in TSV format.')
@click.option('--model',
              'model_file',
              required=True,
              type=click.Path(exists=True, readable=True),
...
def cli(variants_file, model_file, weights_file, reference_file, genome_file, altMinusRef, output):
    ...
    def pybedtoolsIntervalToInterval(interval_pybed):
        ...
        # load variants
        variants = utils.VariantIO.loadVariants(variants_file, fileType=utils.FileType.TSV)
            ...
            if (variant.type == VariantType.DELETION or variant.type == VariantType.INSERTION):
                ...
                if (interval.length > 0):
                    ...
                    if (len(sequence_alt.sequence) == input_length):
                        ...
                        click.echo("Cannot use variant %s because of wrong interval %s has wrong size after InDel Correction" % (str(variant), str(interval)))
                else:
                    click.echo("Cannot use variant %s because interval %s has negative size" % (str(variant), str(interval)))
            # SNV
            else:
