...
def cli(variants_file, model_file, weights_file, reference_file, genome_file, altMinusRef, fileType, output_file):
    ...
    for variant_input in variants_file:
        variants += utils.VariantIO.loadVariants(
            variant_input, fileType=fileType)
    click.echo("Found %d variants" % len(variants))
    if len(variants) == 0:
        click.echo(
            "No variants found. Writing file with header only and exiting...")
    ...
    intervals = pybedtools.BedTool(
        list(map(variantToPybedtoolsInterval, variants)))
