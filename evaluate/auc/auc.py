...
def cli(input_file, label_column, pos_label, prediction_column, output_file):
    ...
    precision, recall, thresholds = metrics.precision_recall_curve(df[label_column], df[prediction_column], pos_label=pos_label)
    auprc = metrics.average_precision_score(df[label_column], df[prediction_column], pos_label=pos_label)

    pd.DataFrame(data={"metric":["AUROC","AUPRC"],"value":[auroc,auprc]}).to_csv(output_file, header=True, index=False, sep="\t", float_format='%.3f')
